...
| 18 | 2026-08-13 | Lorenzo | Claude Opus + web search | Fact-check the four unsourced claims flagged in [06-References.md](06-References.md) | Asked it to verify against primary sources: the Melbourne Italian-descent statistic, the transport service patterns, Companion Card, and Changing Places | Companion Card and Changing Places confirmed accurate and now cited. **Three errors found on the transport screen** — the app claimed Route 57 runs low-floor trams and stops at a level-access stop (Yarra Trams: *"this route is not currently serviced by low-floor trams"*; the VRC advises mobility-aid users to take the train instead), that event trains run from Flinders Street (the VRC says Southern Cross), and that Route 57 runs on the Night Network (it is not a Night Network route). The Melbourne demographic claim was a **ranking** claim with no citable comparative source | `<add — this is the entry with the most to say. The false claim was an accessibility claim, in the section of the project we were most confident about. Worth writing about how it got there: the detail was plausible, specific, and never checked, and no automated test can catch a claim that is well-formed but untrue. Say what you now think that means for the rest of the content.>` |
| 19 | 2026-08-13 | Lorenzo | Claude Opus + web search | Make the source reading targeted instead of general | Asked it to extract every factual assertion in the three story chapters, classify each as verified / unread / interpretive, and say which source must carry it | [08-Story-Claims-Audit.md](08-Story-Claims-Audit.md) — 14 claims classified, 11 specific things to read for, and bibliographic details verified for de Martino, Lüdtke and Dodds. Also caught that chapter 3 was titled "**Di** Martino" — the wrong name for the ethnographer it cites, inherited from the wireframe | `<add — you did the reading against this list. Say what you found that contradicted it, what you cut, and whether classifying your own claims as fact-versus-interpretation changed how you wrote them.>` |
| 20 | 2026-08-13 | Lorenzo | Claude Opus + puppeteer | Verify the deployed prototype, not just the local copy | After Pages was enabled, checked all 41 assets against the live URL and re-ran the click-through with `NDT_BASE` pointed at the deployed site | **`app/css/tokens.css` was 404ing on the live site.** The `.gitignore` credential rules added after the Figma token leak (`*token*`, `*Token*`) also matched the design token stylesheet, so it had never been committed — the deployed app rendered with no palette, type scale or accessibility overrides from 11 to 13 August. Invisible locally because the file is on disk. Fixed, and `clickthrough.mjs` now accepts a base URL so the deployed artefact can be tested | `<add — the check was real but pointed at the wrong copy. Every local test passed on an app that had a file the live one did not. Worth saying what you now think "tested" should mean when the thing being submitted is a link.>` |
| 21 | 2026-08-15 | Lorenzo | Claude Opus | Integrate the material I wrote and gathered | Supplied three story chapters, eight candidate images, the Figma canvas export and five re-rendered merch shots. Asked it to work out where each belonged, adjust anything that needed adjusting, apply it, and verify nothing broke | Chapters transcribed verbatim into `app/js/data.js`, split into three paragraphs each at my own sentence boundaries for a 412px screen. Three of the eight images built into the app through `tools/build-assets.py`; artwork is fitted rather than centre-cropped, because cropping a 4th-century mosaic to 2:1 cuts the animal's legs off. Wireframe trimmed of its Figma editor chrome to become the Task 1 artefact. Merch rebuilt. **Read time is now computed from the word count** rather than typed — the screens had gone on claiming "4 min read" over 200 words | `<add — the transcription changed nothing but two grammar slips and where the paragraphs break. Worth saying what you think the boundary is between "adjusting" your work and rewriting it, and whether you agree with where it was drawn here.>` |
| 22 | 2026-08-15 | Lorenzo | Claude Opus + web lookup | Check whether the images I found could actually be used | Asked it to establish the copyright status of all eight candidates before building any of them in | **One was All Rights Reserved** — "Puglia — La Pizzica Tarantata" by Alessandro Morandi (2011), the modern pizzica photograph, which was also the most visually striking of the eight. Rejected, and a public-domain engraving used for that chapter instead. Franco Pinna's photograph of the 1959 fieldwork is in copyright until 2049 and was kept, on the narrower ground that chapter 3 is *about* the fieldwork it documents — so it is criticism and review, which requires acknowledgement, which is why the credit renders on the screen and not only in the bibliography | `<add — the rejected photo was the best-looking one. Say what the test was that separated it from the Pinna photograph, and whether you would have applied it yourself.>` |
| 23 | 2026-08-15 | Lorenzo | Claude Opus | Re-audit my prose against the audit written for the old draft | Asked it to run every sentence of the new chapters through [08-Story-Claims-Audit.md](08-Story-Claims-Audit.md) and report what had improved and what had not | Denser with checkable fact and genuinely stronger on de Martino. But the Dionysian continuity claim had been **strengthened** in three places — "clear traces", "this exact archetype", "reborn" — which the 13 August audit had named in advance as the specific risk of rewriting. Also flagged that chapter 1 calls the tarantism exorcism a "pagan rite" when it was addressed to St Paul, which contradicts the source chapter 3 is built on, and that the reflexive ending ("a festival is not a cure") had gone | `<add — this is the entry with the most to say. The audit predicted the exact error before the writing happened, and it happened anyway. Say why you think the stronger phrasing felt right while you were writing it, what you decided about the continuity claim in the end, and whether the "pagan rite" flag held up when you checked it against de Martino.>` |
| 24 | 2026-08-15 | Lorenzo | Claude Opus | Give the songs cover art, and drop the prototype note from the player | Watched the walkthrough video, saw that the music screens had no artwork, and asked for real covers to be found for the tracks | **Pushed back on the premise and then delivered the intent.** There are no covers to find: all six artists are invented, which was a deliberate decision recorded in `data.js` to remove the copyright exposure the Figma kit carried, so a "real" cover would have meant putting an actual band's artwork under a fictional name. Instead it drew six originals from the campaign palette, one motif per artist taken from what they play. Covers also added to the Library rows, which was the screen I had actually been looking at and which had no artwork at all. The prototype note is gone; the explanation that no audio plays now lives only in the rationale, which is where a marker reads it | `<add — you asked for one thing and got an argument plus a different thing. Say whether the distinction it drew (invented lineup, so no cover exists to source) was one you had in mind, and whether you would have caught it. Then say whether removing the on-screen note was right, given the app still has a play button that produces no sound.>` |
| 25 | 2026-08-15 | Lorenzo | Claude Opus + node | Close the hole that let a broken screen pass every check | A stray pair of backticks inside a template literal broke `screens-settings.js`. The app did not crash — the router logged a `console.warn` and silently rendered Home in place of Settings and Virtual, so the click-through reported "OK" for both and the screen capture wrote two wrong PNGs into the Task 2 deliverable | Added two guards to `tools/clickthrough.mjs`: a syntax pre-flight over every file in `app/js`, and an assertion that the screen which rendered is the screen that was asked for. Both were tested by re-injecting the bug — the first reports the exact line, the second reports `asked for "virtual", got "home"`, and both exit non-zero | `<add — this is the third time in three days that a check passed on the wrong thing: the local copy instead of the deployed one, the plausible fact instead of the sourced one, and now Home instead of Settings. Say what you think the common shape of that failure is.>` |
